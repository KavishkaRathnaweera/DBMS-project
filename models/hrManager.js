...
class hrManager {
  static async findUserByNIC(NIC){
    ...
    const user =await pool3.query('select * from personal_information where NIC= $1', [NIC])
    return user.rows[0];
...
static async findUserByEmail(email){
  ...
  static async addressTable(address, city, postal_code, country){
    ...
    const countryrow=(await pool3.query(`select * from setCountry($1)`, [country])).rows
    const cityrow=(await pool3.query(`select * from setCity($1,$2)`,[city,countryrow[0].setcountry])).rows
    const addressrow=(await pool3.query(`select setaddress as address_id from setaddress($1,$2,$3)`,[address, cityrow[0].setcity, postal_code])).rows
    return addressrow;
  ...
  static async addEmployee(value) {
    ...
    await pool3.query("BEGIN")
     ...
     const personalDetails =(await pool3.query(` insert into  personal_information(NIC, first_name, middle_name, last_name, gender,birth_day, address_id, email, password) values ($1, $2, $3, $4, $5,$6,$7,$8,$9 ) 
     ...
     const employee=(await pool3.query(`insert into Employee (employee_id ,branch_name, job_title, dept_name, paygrade_level, e_status_name) values($1,$2, $3, $4, $5, $6)
    ...
    await hrManager.customAttributes(value)
     ...
     const empEmergency = (await pool3.query(`INSERT INTO emergency_contact_details(employee_id,relative_name,contact_no) values($1,$2, $3)
                                          `,[personalDetails.employee_id,value.first_name,value.phone]));
     const empPhone = (await pool3.query(`INSERT INTO employee_phone_number(employee_id,phone) values($1,$2)
