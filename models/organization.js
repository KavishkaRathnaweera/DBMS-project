...
class Organization {
  ...
  static async getBranch(branch_name) {
    const branch = await db.query(
      `
        select * from branch left outer join address using(address_id) left outer join city using(city_id) left outer join country using(country_id)  where branch_name=$1`,
      [branch_name]
    );
  ...
  static async setLeave(paygrade_level, anual, casual, maternity, no_pay) {
    ...
    return (
      await db.query(`call updateJupitorLeaves($1 ,$2, $3, $4 ,$5 )`, [
        paygrade_level,
        anual,
        casual,
        maternity,
  ...
  static async getAllBranchesWithAddress() {
    return (
      await db.query(
        `select * from branch left outer join address using(address_id) left outer join city using(city_id) left outer join country using(country_id)`
      )
    ).rows;
  }

  static async addBranch(branch_name, address, city, postal_code, country) {
    const addressrow = await User.addressTable(
      address,
      city,
      postal_code,
      country
    );
    return await db.query(`insert into branch values($1, $2)`, [
      branch_name,
  ...
  static async deleteCustomAttribute(columnName) {
    ...
    return await db.query(
      `alter table personal_information_custom drop column ${columnName}`
    );
  }
  ...
  static async addPayGrade(paygrade_level, description, requirement) {
    return await db.query(
      "insert into pay_grade (paygrade_level , description, requirement) values($1,$2,$3) ",
      [paygrade_level, description, requirement]
    );
  }
  static async getPayGrade(paygrade_level) {
    return (
      await db.query("select * from pay_grade where paygrade_level=$1", [
        paygrade_level,
      ])
    ).rows[0];
  }
  static async setPayGrade(paygrade_level, description, requirement) {
    return await db.query(`call updateJupitorPayGrade($1 ,$2, $3 )`, [
      paygrade_level,
      description,
      requirement,
    ]);
  ...
  static async addEmployeeStatus(e_sataus_name, duration, description) {
    return await db.query(
      `insert into employee_status (e_status_name, duration, description) values($1, $2, $3)`,
      [e_sataus_name, duration, description]
    );
  }
  static async getEmployeeState(e_status_name) {
    return (
      await db.query("select * from employee_status where e_status_name=$1", [
        e_status_name,
      ])
    ).rows[0];
  }
  static async setEmployeeStatus(e_status_name, duration, description) {
    return await db.query("call updateJupitorEmployeeStatus($1, $2, $3)", [
      e_status_name,
      duration,
      description,
    ]);
  ...
  static async getJobType(job_title) {
    return (
      await db.query("select * from job_type where job_title=$1", [job_title])
    ).rows[0];
  }
  static async addJobType(
    job_title,
    description,
    req_qualification,
    prerequisites
  ) {
    return await db.query(`insert into job_type values($1, $2, $3, $4)`, [
      job_title,
      description,
      req_qualification,
      prerequisites,
  ...
  static async setJobType(
    job_title,
    description,
    req_qualification,
    prerequisites
  ) {
    return await db.query("call updateJupitorJobs($1, $2, $3, $4)", [
      job_title,
      description,
      req_qualification,
      prerequisites,
  ...
  static async getHRData(branch_name) {
    return (
      await db.query(
        "select * from personal_information left outer join employee using(employee_id) left outer join address using(address_id) left outer join city using(city_id) left outer join country using(country_id)  where branch_name=$1",
        [branch_name]
      )
    ).rows[0];
  }
  static async getEmplyeeCount(branch_name) {
    return (
      await db.query(
        "select count(*) from personal_information left outer join employee using(employee_id) where branch_name=$1",
        [branch_name]
      )
    ).rows[0];
  }
  static async setBranch(branch_name, address, city, postal_code, country) {
    ...
    return await db.query(`call updateJupitorBranch($1, $2)`, [
      branch_name,
      addressrow[0].address_id,
    ]);
  }
