    ...
    static async getEmployees(branch,department,jobtype,user,querySelect){
      ...
      switch(querySelect){
        case 1:
          ...
          select employee_id,nic,first_name,last_name from personal_information join employee using(employee_id) where branch_id = $1 and job_title != $2`,[branch,user])).rows;
          return employees;
        case 2:
            employees=(await db.query(`
            select employee_id,nic,first_name,last_name from personal_information join employee using(employee_id) 
            where branch_id = $1 and job_title  = $2`,[branch,jobtype])).rows;
            return employees;
        case 3:
            employees=(await db.query(`
            select employee_id,nic,first_name,last_name from personal_information join employee using(employee_id) 
            where branch_id = $1 and dept_name = $2 and job_title != $3`,[branch,department,user])).rows;
            return employees;
        case 4:
            employees=(await db.query(`
            select employee_id,nic,first_name,last_name from personal_information join employee using(employee_id) 
            where branch_id = $1 and dept_name = $2 and job_title  = $3 `,[branch,department,jobtype])).rows;
            return employees;
        default:
...
static async getEmployeesToaddSupervisorT(branch,department,user){
    ...
    where branch_id = $1 and dept_name = $2 and job_title != $3 
...
static async saveSupervisorGroup(supervisor_id,supervisorGroupemployeeIDs){
    ...
    const result=(await db.query(`
    CALL addToSupervisorT($1,$2)`,[supervisorGroupemployeeIDs,supervisor_id])).rows;
    return result;
} 
static async getSupervisors(){
    const result=(await db.query(`
    select employee_id,nic,first_name,last_name from personal_information join employee using(employee_id)
    where supervisor = true`)).rows;
    return result;
}
